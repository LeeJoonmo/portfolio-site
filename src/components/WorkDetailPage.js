  ...
  _renderWork = props => {
    ...
    if (currentWork === undefined) {
      return (
        ...
        >
          Nothing here, Please go to Main Page :D
        </div>
      );
    } 
    else return currentWork.page;
  };
    ...
    return (
      <div>
        ...
        <div className="work-button work-detail-container">
          {currentIndex > 0 ? (
            <Link
              className="work-button-prev"
              onClick={() => {
                window.scrollTo(0, 0);
              }}
              to={`/workDetail/${this.state.worksData[currentIndex - 1].name}`}
            >
              <div>Prev work</div>
              <div className="work-button-title">
                {this.state.worksData[currentIndex - 1].title}
              </div>
            </Link>
          ) : null}
          {currentIndex === this.state.worksData.length - 1 ? null : (
            <Link
              className="work-button-next"
              onClick={() => {
                window.scrollTo(0, 0);
              }}
              to={`/workDetail/${this.state.worksData[currentIndex + 1].name}`}
